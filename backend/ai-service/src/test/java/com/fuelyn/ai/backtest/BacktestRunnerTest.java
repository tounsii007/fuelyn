...
/**
 * Smoke test for the backtesting runner using a synthetic two-week fixture. We don't assert the
 * heuristic is "good" here — that's what the report itself measures — only that the framework runs
...
class BacktestRunnerTest {
    ...
    /**
     * Build a fixture mimicking the German diurnal pattern: roughly sinusoidal drop in the evening,
     * slight rise overnight, +1 ct trend across the period. Two stations with a small brand offset.
     */
    ...
    private static String buildStation(String brand, double base, double brandOffset) {
        ...
        for (int hour = 0; hour < 14 * 24; hour++) {
            ...
            String ts =
                    OffsetDateTime.ofInstant(t0.plus(Duration.ofHours(hour)), ZoneOffset.UTC)
                            .toString();
            if (hour > 0) sb.append(",");
            sb.append("{ \"ts\": \"")
                    .append(ts)

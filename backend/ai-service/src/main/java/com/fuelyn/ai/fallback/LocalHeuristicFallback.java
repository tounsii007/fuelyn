...
import com.fuelyn.ai.signals.AffiliateRebate;
import com.fuelyn.ai.signals.BayesianPrior;
import com.fuelyn.ai.signals.BrandBaseline;
import com.fuelyn.ai.signals.EwmaChangePoint;
import com.fuelyn.ai.signals.PriceFreshness;
import com.fuelyn.ai.signals.RoutePenalty;
import com.fuelyn.ai.signals.StationForecaster;

/**
 * Multi-signal advisor heuristic.
 *
 * <p>Eight independent signals contribute to a buy/wait vote. Each one lives in roughly [-1, +1]
 * (positive = buy bias) and is multiplied by an empirically tuned weight. Output now also exposes
 * the per-signal contributions through {@link AIAdvisorResponse#breakdown()} so the client can
 * render an "explainable AI" view and developers can debug any single decision.
 *
 * <h3>Signals</h3>
 *
 * <ol>
 *   <li><b>Brand-relative z-score</b> — cheapest station within its brand cluster (handles the
 *       systemic Aral/Shell premium)
 *   <li><b>Spread factor</b> — wide market dispersion → real saving
 *   <li><b>EWMA trend</b> — direction + strength of last-24 h slope
 *   <li><b>Change-point</b> — recent drop = market just reset
 *   <li><b>Day curve</b> — Mon..Sun cheap/expensive map
 *   <li><b>Hour curve</b> — diurnal pattern
 *   <li><b>Bayesian posterior</b> — prior bucket vs. observed
 *   <li><b>Tank urgency</b> — when we know fuel level, low tank pulls toward buy_now (waiting
 *       becomes risky)
 * </ol>
 *
 * <p>Each price is also weighted by {@link PriceFreshness} so stale Tankerkönig data exerts less
...
public final class LocalHeuristicFallback {
    ...
    public static AIAdvisorResponse analyze(AIAdvisorRequest request, Clock clock) {
        ...
        BayesianPrior.Posterior posterior =
                BayesianPrior.at(LocalDateTime.now(clock), observedDiscountCt);
        ...
        double zContribution = zClamped < 0 ? -zClamped * zWeight : -zClamped * 0.8;
        ...
        // 6) Change-point
        double changePointContribution =
                switch (trend.changePoint()) {
        ...
        if (shouldBuy) {
            headline =
                    trend.changePoint() == EwmaChangePoint.ChangePoint.RECENT_DROP
                            ? "Jetzt — Preise gerade gefallen"
                            : trend.direction() == EwmaChangePoint.Direction.RISING
                                    ? "Jetzt tanken — Trend steigt"
                                    : detourPaysOff ? "Jetzt tanken (kurzer Weg)" : "Jetzt tanken!";
            explanation =
                    String.format(
                                    Locale.GERMANY,
                                    ...
                                    brandStats.cheapestBrandZ() < 0 && brandStats.globalStdDev() > 0
                                            ? String.format(
                                                    Locale.GERMANY,
                                                    " (%.1f σ unter dem Brand-Schnitt)",
                                                    -brandStats.cheapestBrandZ())
                                            : "",
                                    isCheapDay(dow)
                                            ? "Dienstag/Mittwoch sind erfahrungsgemäß günstig. "
                                            : "",
                                    trend.direction() == EwmaChangePoint.Direction.RISING
                                            ? String.format(
                                                    Locale.GERMANY,
            ...
            explanation =
                    "Freitag/Samstag sind erfahrungsgemäß teurer. Versuche es Dienstag oder Mittwoch.";
        } else {
            headline = "Warten lohnt sich";
            explanation =
                    String.format(
        ...
        AIAdvisorResponse.BestStation bestStation =
                new AIAdvisorResponse.BestStation(
                        cheapest.stationName(),
                        String.format(
                                Locale.GERMANY,
                                "Günstigster Preis: %.3f €, %.1f km entfernt",
                                minPrice,
                                cheapest.distance()));
        AIAdvisorResponse.BestStation effectiveBestDto =
                new AIAdvisorResponse.BestStation(
                        ...
                        String.format(
                                ...
                                userOptedIntoLoyalty
                                                && AffiliateRebate.perLitre(
                                                                effectiveBest.brand(), true)
                                                        > 0
                                        ? " − Loyalty-Bonus"
                                        : ""));
    ...
    /**
     * Effective €-per-litre at a station, including round-trip drive cost amortised over the
     * fill-up volume and any loyalty rebate. Uses the user's actual consumption when known; falls
     * back to a 0.18 €/km flat estimate.
     */
    static double effectivePrice(
            AIAdvisorRequest.StationPrice p, int liters, AIAdvisorRequest req) {
    ...
    /**
     * Tank urgency in [0,1]. 0 = full tank, 1 = below 15 % (drive to nearest, don't wait). Linear
     * taper between.
     */
    ...
    private static String priceOutlookText(
            EwmaChangePoint.Result trend,
            DayOfWeek dow,
            int hour,
            BayesianPrior.Posterior posterior) {
    ...
    private static String tipText(
        ...
        if (hour >= 18 && hour <= 20)
            return "Du tankst bereits im günstigen Abendfenster (18–20 Uhr).";
        if (hour <= 9 && hour >= 6)
            return "Tipp: morgens ist es typisch teurer — lieber zwischen 18 und 20 Uhr tanken.";
        if (isCheapDay(dow))
            return "Heute ist ein erfahrungsgemäß günstiger Tag — abends 18–20 Uhr besonders.";
    ...
    private static AIAdvisorResponse defaultResponse(Integer fillUpLiters) {
        ...
        return new AIAdvisorResponse(
                ...
                "Preise fallen typischerweise dienstags.",
                0,
                "low",
                null,
                "Unbekannt",
                String.format(
                        Locale.GERMANY,
                        "Vergleiche Preise vor dem Tanken (typische Füllmenge: %d L).",
                        liters),
                false,
                false);
    }
    ...
    public static double effectivePriceOf(
            AIAdvisorRequest.StationPrice p, int liters, AIAdvisorRequest req) {

 ...
 *
 * <p>Each subscriber holds an {@link SseEmitter} that we add to the
 * fan-out list. On a new anomaly we iterate the list and write the
 * event; failed writes prune the dead emitter.</p>
 *
 * <p>Concurrency: {@link CopyOnWriteArrayList} is fine at our scale
 * (low write rate, modest reader count). For hundreds of concurrent
 * subscribers we'd switch to a {@code ConcurrentLinkedQueue} or a
 * Project Reactor {@code Sinks.Many} broadcaster.</p>
 *
 * <p>This component is memory-only — no Redis pub-sub, no Kafka
 * fan-out. A multi-instance deployment will need to broadcast across
 * replicas (Kafka topic {@code fuelyn.ai.anomalies.v1}); for now a
 * single ai-service instance is fine.</p>
 */
@Component

 ...
 *
 * <p>When the user supplies a destination, a station's relevant "distance" is no longer the
 * as-the-crow-flies distance from the current position — it's the <i>extra</i> distance you'd drive
 * versus going straight to the destination.
 *
 * <p>Without a routing engine we approximate the detour as {@code (origin → station) + (station →
 * destination) − (origin → destination)}, all using the haversine formula. That's an over-estimate
 * on curving roads but a good first-order signal — and zero-cost.
 */
public final class RoutePenalty {
    ...
    public static Detour compute(
        ...
        double b =
                haversine(originLat, originLng, station.lat(), station.lng())
    ...
    private static double haversine(double lat1, double lng1, double lat2, double lng2) {
        ...
        double a =
                s1 * s1 + Math.cos(Math.toRadians(lat1)) * Math.cos(Math.toRadians(lat2)) * s2 * s2;

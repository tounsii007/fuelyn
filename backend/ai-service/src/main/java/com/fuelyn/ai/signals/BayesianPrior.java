 ...
 *
 * <p>Empirical observation from German fuel-price datasets (MTS-K 2022–2025): each (day-of-week,
 * hour) bucket has a stable mean discount versus the daily average and a stable standard deviation.
 * For example, Wed 19:00 averages ≈ −4 ct vs. the day's mean with σ ≈ 1.2 ct.
 *
 * <p>This class encodes that table as a static prior. Combined with a current observation (the
 * spread we see in the live request) it yields a posterior probability that "now" is a buying
 * opportunity. No external data file required — the values are baked-in conjugate priors derived
 * from public Tankerkönig analyses.
 */
public final class BayesianPrior {

    /**
     * Posterior over "is this time of week a buy moment?" expressed as a soft signal in [-1, +1]:
     * +1 → typically very cheap window, strong bias to buy 0 → average −1 → typically expensive
     * window, strong bias to wait
     */
    public record Posterior(double tilt, double priorMean, double priorStd) {}

    /**
     * Approximate mean discount (€) vs. daily mean for each (DayOfWeek, hour) bucket. Negative =
     * cheaper than day mean. Compact encoding: rows = day (1..7), cols = hour (0..23). Values in
     * cents (×0.01 €) for readability.
     */

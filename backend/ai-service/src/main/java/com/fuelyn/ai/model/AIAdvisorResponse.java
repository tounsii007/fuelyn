 ...
 *
 * <p>The first eleven fields preserve the original contract — clients coded against them keep
 * working. The optional {@code breakdown}, {@code perStationForecast}, and {@code effectiveBest}
 * fields enrich the response with explainability and per-station forecasts; they are suppressed
 * (not emitted) when null.
 */
...
public record AIAdvisorResponse(
    ...
    /**
     * Predicted price quantiles for one station over the next 24 h. {@code p10} = optimistic
     * (chance the price will fall to this), {@code p50} = median expectation, {@code p90} =
     * pessimistic.
     */
    @JsonIgnoreProperties(ignoreUnknown = true)
    public record StationForecast(
            String stationName, double currentPrice, double p10, double p50, double p90) {}

    /**
     * Legacy 11-arg constructor — keep this so all existing call sites (LocalHeuristicFallback,
     * OpenAIAdvisorService, tests) compile without modification. The new optional fields default to
     * null.
     */

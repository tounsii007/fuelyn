...
/**
 * Premium-tier enrichment via OpenAI's chat completions API (default: {@code gpt-4o-mini}).
 *
 * <p>Same prompt as the Ollama backend — only the transport, the auth header, and the JSON-mode
 * flag differ. In the tiered orchestrator this backend runs only when local Ollama is unavailable
 * or fails, so token spend is bounded by Ollama's hit rate.
 *
 * <p>This bean is created only when an API key is present, so the absence of OpenAI configuration
 * in dev does not break startup.
 */
@Component
@ConditionalOnProperty(
        prefix = "fuelyn.ai.openai",
        name = "enabled",
        havingValue = "true",
...
public class OpenAIEnrichmentBackend implements EnrichmentBackend {
    ...
    /**
     * Approximate USD cost per output token for the configured model. Keeps the cost-estimate log
     * accurate as OpenAI repricings happen. Default matches gpt-4o-mini (60 ¢ / 1M output tokens, ≈
     * 0.0000006 USD/tok). Operators bumping the model should update this in lockstep.
     */
    ...
    public OpenAIEnrichmentBackend(
        ...
        this.restTemplate =
                builder.connectTimeout(Duration.ofMillis(Math.min(15_000, timeoutMs)))
        ...
        log.info(
                "OpenAIEnrichmentBackend ready — model={}, costPerToken={}, key={}",
                model,
                costPerToken,
    ...
    /**
     * Phase A2 — Bulkhead caps concurrent OpenAI calls to bound token spend on bot storms. Falls
     * through to baseline when over capacity.
     */

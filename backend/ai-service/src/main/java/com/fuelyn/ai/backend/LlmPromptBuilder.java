 ...
 *
 * <p>Both Ollama and OpenAI backends produce identical prompts so that switching between providers
 * has no semantic impact on the output — only on quality and latency.
 *
 * <h3>Design</h3>
 *
 * <ul>
 *   <li>The system prompt fixes the role, the JSON schema, and the hard rule that the model must
 *       NOT touch structured fields.
 *   <li>The user prompt embeds the heuristic verdict as <b>fixed facts</b> the model must respect,
 *       plus the raw market data so the model has enough context to write something useful.
 *   <li>Output is a small JSON object with five string fields. A 3B-parameter model handles this
 *       size reliably.
 * </ul>
 */
public final class LlmPromptBuilder {
    ...
    /** Returns the shared system prompt that constrains the model to narrative enrichment only. */
    ...
    /**
     * Builds the user prompt embedding the heuristic baseline as fixed facts plus the raw market
     * data for context.
     */
    public static String userPrompt(
            AIAdvisorRequest request, AIAdvisorResponse baseline, ObjectMapper mapper) {
        ...
        sb.append("- savingsEstimate: ")
                .append(String.format(Locale.GERMANY, "%.2f", baseline.savingsEstimate()))
                .append(" €\n");
        sb.append("- confidence: ").append(baseline.confidence()).append("\n");
        if (baseline.bestStation() != null) {
            sb.append("- bestStation: ")
                    .append(baseline.bestStation().name())
                    .append(" — ")
                    .append(baseline.bestStation().reason())
                    .append("\n");
        }
        sb.append("- bisherige Erklärung (kannst du verbessern): ")
                .append(baseline.explanation())
                .append("\n\n");

        sb.append("Markt-Kontext:\n");
        sb.append("- Kraftstoff: ").append(request.fuelType()).append("\n");
        sb.append("- Tankmenge: ")
                .append(request.fillUpLiters() == null ? 50 : request.fillUpLiters())
                .append(" L\n");
        sb.append("- Wochentag: ")
                .append(DAYS_DE[LocalDate.now().getDayOfWeek().getValue() - 1])
                .append("\n");
    ...
    /**
     * Strip control characters and obvious prompt-injection trigger words. Note: this is
     * defense-in-depth on top of input validation at the controller layer.
     */
    private static String sanitize(String input) {
        return input.replaceAll("[\\x00-\\x1F\\x7F]", "")
                .replaceAll(
                        "(?i)(ignore previous|forget instructions|pretend you are)", "[filtered]");
    }

    /**
     * Merge the LLM's narrative-only response into the heuristic baseline. Any field missing or
     * empty in the LLM output falls back to the baseline value, so a partial answer never produces
     * blanks in the UI.
     */
    public static AIAdvisorResponse mergeNarrative(
            AIAdvisorResponse baseline, NarrativeFields llm) {
    ...
    /** Narrative-only fields produced by the LLM. Mirrors the JSON schema in the system prompt. */

...
import com.tankpilot.common.security.SecretPolicy;

/**
 * Externalized security configuration properties shared across TankPilot services.
 *
 * <p>Bound to the {@code tankpilot.security} prefix in {@code application.yml}.
 ...
 */
@Component
@ConfigurationProperties(prefix = "tankpilot.security")
@Validated
public class SecurityProperties {

    /** Shared HMAC secret for request-body signing (service-to-service). */
    @NotBlank(message = "tankpilot.security.hmac-secret must be set")
    ...
    void validateSecrets() {
        SecretPolicy.requireStrong("tankpilot.security.hmac-secret", hmacSecret);

        if (jwtPublicKey == null || jwtPublicKey.isBlank()) {
            throw new IllegalStateException(
                    "Refusing to start: tankpilot.security.jwt-public-key is not set. "
        ...
        if (jwtPrivateKey != null && !jwtPrivateKey.isBlank()) {
            if (jwtPrivateKey.length() < 256) {
                throw new IllegalStateException(
                        "tankpilot.security.jwt-private-key is too short to be a real RSA key.");
            }
        }
        for (int i = 0; i < apiKeys.size(); i++) {
            String key = apiKeys.get(i);
            if (key != null && !key.isBlank()) {
                SecretPolicy.requireStrong("tankpilot.security.api-keys[" + i + "]", key);
            }
        }
    }

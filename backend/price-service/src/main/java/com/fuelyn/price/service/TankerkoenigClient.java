 ...
 *
 * <ul>
 *   <li>{@code @CircuitBreaker} &mdash; Opens after 50% failures in 10-call window, waits 30s
 *   <li>{@code @Retry} &mdash; 3 attempts with exponential backoff (2s, 4s, 8s)
 *   <li>{@code @RateLimiter} &mdash; Max 30 calls/minute to respect API limits
 * </ul>
 */
@Service
public class TankerkoenigClient implements FuelStationClient {
    ...
    public List<TankerkoenigResponse.Station> searchStations(
        ...
        String url =
                baseUrl
                        + "/json/list.php?lat={lat}&lng={lng}&rad={rad}&type=all&apikey={key}&sort=dist";

        log.debug("Fetching stations: lat={}, lng={}, radius={}km", lat, lng, radiusKm);
        long start = System.currentTimeMillis();

        TankerkoenigResponse.ListResponse response =
                restTemplate.getForObject(
                        url, TankerkoenigResponse.ListResponse.class, lat, lng, radiusKm, apiKey);
        ...
        List<TankerkoenigResponse.Station> stations =
                response.stations() != null ? response.stations() : Collections.emptyList();

        log.info(
                "Fetched {} stations in {}ms (lat={}, lng={}, rad={}km)",
                stations.size(),
                duration,
                lat,
                lng,
    ...
    private List<TankerkoenigResponse.Station> searchStationsFallback(
        ...
        log.warn(
                "[CircuitBreaker] Tankerkoenig API unavailable: {}. Returning empty.",
                t.getMessage());
        return Collections.emptyList();
    }
    ...
    public TankerkoenigResponse.Station fetchStationDetail(String stationId) {
        ...
        TankerkoenigResponse.DetailResponse response =
                restTemplate.getForObject(
                        url, TankerkoenigResponse.DetailResponse.class, stationId, apiKey);

        if (response == null || !response.ok()) {
            log.error(
                    "Station detail API error: {}", response != null ? response.message() : "null");
            return null;
        }
     ...
     *
     * <p>Decorated with the same {@code external-api} rate limiter as the sibling search/detail
     * methods — without it, a bursty caller (e.g. a favourites-list refresh that fans out to 50
     * stations in 5 batches) could blow Tankerkönig's per-key quota independently of the limit
     * applied everywhere else in this service.
     */
    ...
    public Map<String, TankerkoenigResponse.PriceEntry> fetchPrices(List<String> stationIds) {
        ...
        TankerkoenigResponse.PricesResponse response =
                restTemplate.getForObject(

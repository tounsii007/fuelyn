 ...
 *
 * <p>Default schedule: 03:30 daily — chosen to run after the existing
 * data-cleanup job at 03:00 (see {@link PriceCollectionScheduler}) so
 * the two jobs don't both fight for the connection pool, and well
 * before the morning collection burst.</p>
 *
 * <p>Guarded by a {@link SchedulerLock} so a clustered deployment
 * does not run the loop twice. Disabled by default; enable via env
 * {@code REGRET_BACKFILL_ENABLED=true} once the Flyway migration is
 * applied and ai-service has been writing rows for a while.</p>
 */
@Component
...
public class RegretBackfillScheduler {
    ...
    public void runDaily() {
        ...
        try {
            ...
            log.info("Regret backfill — nightly pass done, {} rows received a regret value", filled);
        } catch (Exception e) {

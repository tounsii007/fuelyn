 ...
 *
 * <p>Provides efficient queries for price history, statistics, and day-of-week pattern analysis.
 * Custom queries use JPQL with proper indexing on {@code (station_id, fuel_type, timestamp)}.
 */
@Repository
public interface PriceSnapshotRepository extends JpaRepository<PriceSnapshot, Long> {
    ...
    /**
     * Returns the most recent snapshot for a (station, fuel) pair, if any. Used by the streaming
     * layer to compute price-change deltas without loading the full history.
     */
    Optional<PriceSnapshot> findFirstByStationIdAndFuelTypeOrderByTimestampDesc(
            String stationId, String fuelType);

    /**
     * Batch variant of {@link #findFirstByStationIdAndFuelTypeOrderByTimestampDesc} for the
     * collection hot path: returns the most-recent snapshot per (stationId, fuelType) across an
     * entire area in a single round-trip.
     *
     * <p>The correlated subquery is portable across Postgres and H2 — both recognise the equality
     * on max(timestamp) per group and use the {@code (station_id, fuel_type, timestamp)} unique
     * index for the inner aggregate. This replaces N×M individual lookups (250 stations × 3 fuel
     * types = 750 queries per area) with one.
     */
    @Query(
    ...
    @Query(
            "SELECT AVG(p.price) FROM PriceSnapshot p "
    ...
    /**
     * Aggregated area statistics grouped by station. Returns Object[] with: [stationId, avgPrice,
     * minPrice, maxPrice, count].
     */
    @Query(
            "SELECT p.stationId, AVG(p.price), MIN(p.price), MAX(p.price), COUNT(p) "
    ...
    List<Object[]> findAreaStats(
            @Param("fuelType") String fuelType, @Param("after") LocalDateTime after);

    /** Day-of-week price pattern analysis using DAYOFWEEK function. */
    @Query(
            "SELECT FUNCTION('DAYOFWEEK', p.timestamp) AS dow, AVG(p.price) AS avgPrice "
                    + "FROM PriceSnapshot p "
                    + "WHERE p.stationId = :stationId AND p.fuelType = :fuelType AND p.timestamp > :after "
     ...
     *
     * <p>Used internally as the inner step of the chunked variant {@code
     * PriceCollectorService.cleanupOldData}. A single unbounded DELETE on a 90-day-deep table can
     * hold a Postgres lock on price_snapshots for minutes, blocking concurrent inserts from the
     * collection cycle. Prefer {@link #deleteOldestBatch} from the service layer.
     */
    @Modifying
    ...
    /**
     * Returns the IDs of up to {@code limit} oldest snapshots beyond the retention cutoff. The
     * service deletes by ID in chunks so the whole purge fits inside short transactions that don't
     * starve the polling writes.
     */
    ...
    /**
     * Bulk delete by primary-key list. Uses the implicit pk-index for O(log n) per row and stays
     * well below typical Postgres lock-wait thresholds when the chunk size is conservative (~5k).
     */
    @Modifying

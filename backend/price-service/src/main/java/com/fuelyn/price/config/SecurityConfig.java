 ...
 * authorization check and rejects unsigned callers with 401. This is
 * defense-in-depth on top of Docker network isolation — the service ports
 * are never published to the host, but the filter ensures that anything
 * reaching {@code /api/v1} actually came through the gateway.</p>
 */
@Configuration
@EnableWebSecurity

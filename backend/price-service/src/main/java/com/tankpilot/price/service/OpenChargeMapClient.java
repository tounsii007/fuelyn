...
import com.tankpilot.price.model.dto.ChargingStationResponse;
import com.tankpilot.price.model.dto.ChargingStationResponse.*;
...
public class OpenChargeMapClient implements ChargingStationClient {
    ...
    private static final Map<Integer, String> CONNECTION_TYPE_MAP = Map.ofEntries(
    ...
    public OpenChargeMapClient(
            ...
            @Value("${tankpilot.openchargemap.api-key:}") String apiKey
    ) {
    ...
    public List<ChargingStation> searchChargingStations(double lat, double lng, double radiusKm) {
        StringBuilder urlBuilder = new StringBuilder(OCM_BASE)
                .append("?output=json")
                .append("&latitude=").append(lat)
                .append("&longitude=").append(lng)
                .append("&distance=").append(Math.min(radiusKm, 100))
        ...
        ResponseEntity<List<OCMResult>> response = restTemplate.exchange(
        ...
        List<ChargingStation> stations = results.stream()
    ...
    private List<ChargingStation> searchChargingFallback(double lat, double lng, double radiusKm, Throwable t) {
        log.warn("[CircuitBreaker] OpenChargeMap unavailable: {}. Returning empty.", t.getMessage());
        return Collections.emptyList();
    }

    private ChargingStation mapToChargingStation(OCMResult r) {
        OCMAddress addr = r.AddressInfo();
        List<Connection> connections = r.Connections() != null
        ...
        return new ChargingStation(
                ...
                connections,
                r.StatusType() == null || r.StatusType().IsOperational() == null || r.StatusType().IsOperational(),
                r.UsageCost(),
    ...
    private Connection mapConnection(OCMConnection c) {
        String typeName = CONNECTION_TYPE_MAP.getOrDefault(c.ConnectionTypeID(),

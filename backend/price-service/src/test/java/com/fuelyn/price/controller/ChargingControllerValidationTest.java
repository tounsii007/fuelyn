...
/**
 * Verifies bean-validation annotations on {@link ChargingController} produce HTTP 400 for
 * out-of-range coordinates.
 *
 * <p>Uses a standalone MockMvc setup (no ApplicationContext), which sidesteps Mockito's byte-buddy
 * inline-mock-maker issues with newer JDKs and keeps this test tiny and fast.
 */
@ExtendWith(MockitoExtension.class)
class ChargingControllerValidationTest {

    @Mock private OpenChargeMapClient chargeMapClient;
    ...
    void setUp() {
        ...
        this.mockMvc =
                MockMvcBuilders.standaloneSetup(proxied)
    ...
    void missingLat_returns400() throws Exception {
        mockMvc.perform(get("/api/v1/prices/charging").param("lng", "13.40"))
                .andExpect(status().isBadRequest());
    }
}

 ...
 *
 * <p>The full broker round-trip is covered separately by the integration test fixture (Linux-only —
 * embedded Kafka has a known Windows + Java 21 loopback bug that flakes the run). These tests prove
 * the publisher contract with a mocked {@link KafkaTemplate}:
 *
 * <ul>
 *   <li>Sends to the configured topic.
 *   <li>Uses {@code stationId} as the partition key.
 *   <li>Wraps the event in a CloudEvents-style envelope.
 *   <li>Is a silent no-op when no template is wired.
 *   <li>Swallows synchronous exceptions so the persistence path can never be broken by the
 *       streaming layer.
 *   <li>Serialises to JSON without losing fields when run through Spring's {@code JsonSerializer}
 *       with our shared mapper.
 * </ul>
 */
class PriceEventPublisherTest {
    ...
    void publishWithNullTemplate_isSilentNoOp() {
        ...
        publisher.publish(
                PriceUpdatedEvent.forUpdate(
                        "X", "X", "x", "e10", 1.7, null, Instant.now(), 0.0, 0.0, null));
    }
    ...
    void publishSwallowsSynchronousExceptions() {
        ...
        publisher.publish(
                PriceUpdatedEvent.forUpdate(
                        "Y", "Y", "y", "e10", 1.7, null, Instant.now(), 0.0, 0.0, null));
